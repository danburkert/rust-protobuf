...
pub trait CodedOutputStream: Write {
    ...
    fn write_raw_byte(&mut self, byte: u8) -> ProtobufResult<()>;

    fn write_raw_bytes(&mut self, bytes: &[u8]) -> ProtobufResult<()>;
    ...
    fn write_raw_varint64(&mut self, value: u64) -> ProtobufResult<()> {
        let mut temp = value;
        loop {
            if (temp & !0x7Fu64) == 0 {
...
impl<W> CodedOutputStream for W where W: Write {
    fn write_raw_byte(&mut self, byte: u8) -> ProtobufResult<()> {
        self.write_all(&[byte]).map_err(ProtobufError::IoError)
    }

    fn write_raw_bytes(&mut self, bytes: &[u8]) -> ProtobufResult<()> {
        self.write_all(bytes).map_err(ProtobufError::IoError)
    }
}
